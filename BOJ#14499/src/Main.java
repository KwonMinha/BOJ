...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		Dice curDice = new Dice(0, 1, 2, 3, 4, 5); // 초기 주사위 면의 상태 - top, up, down, left, right, bottom 순서 
		...
		for(int i = 0; i < K; i++) {
			...
			if(nx < 0 || nx >= N || ny < 0 || ny >= M) { // 범위를 벗어나는 경우 -> 해당 명령 무시 
				continue;
			}
			...
			if(map[nx][ny] == 0) { // 이동한 칸에 쓰여 있는 수가 0이면, 주사위의 바닥면에 쓰여 있는 수가 칸에 복사
