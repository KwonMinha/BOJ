...
public class Main3 {
	...
	public static void spring() {
		while(!pq.isEmpty()) {
			...
			if(tree.age <= map[tree.x][tree.y]) { // 나이만큼 양분을 먹을 수 있다면 -> 나이 1 증가 -> 살아있는 나무 리스트에 저장 
				map[tree.x][tree.y] -= tree.age; // 양분 먹은만큼 빼기 
				aliveList.add(new Tree(tree.x, tree.y, tree.age + 1));
			} else { // 양분 부족 -> 죽음 -> 죽은 나무 리스트에 저장 
	...
	public static void summer() {
		...
		deadList.clear(); // remove() 삭제하면 시간 초과 - 그냥 index로 접근만 하고 확 밀어버려야 함 
	}
	...
	public static void fall() {	
		for(int i = 0; i < aliveList.size(); i++) {
			...
			pq.offer(new Tree(tree.x, tree.y, tree.age)); // 5의 배수가 아니라도 살아있으니 pq에 추가 
		}

		aliveList.clear(); // remove() 삭제하면 시간 초과 - 그냥 index로 접근만 하고 확 밀어버려야 함 
	}
