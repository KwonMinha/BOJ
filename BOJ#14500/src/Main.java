...
public class Main {
	...
	// (0, 0) 기준 - ㅜ로 나올 수 있는 4가지 모양 미리 만듬 
	...
	public static void main(String[] args) {
		...
		N = sc.nextInt();
		M = sc.nextInt();
		map = new int[N+1][M+1]; // (0, 0) 기준 ㅜ 모양이 한칸씩 범위 밖으로 나가는 것을 고려해 map크기 +1 
	...
	public static void dfs(int depth, int x, int y, int sum) {
		if(depth == 4) { // 4개의 정사각형이 모두 놓아진 경우 종료 
		...
		for(int i = 0; i < 4; i++) {
			...
			if(nx < 1 || nx > N || ny < 1 || ny > M) // 범위 넘어가면 pass 
				continue;
