	...
	public static void dfs(int n, int depth, int cnt) {
		...
		for(int i = 1; i < H+1; i++) {
			for(int j = 1; j < N; j++) { // N-1에서 가로선 왼->오 (x, y) -> (x, y+1) / 오->왼 (x, y+1) -> (x, y) 다 설정하기 때문에 N까지만 
				if(j+1 < N+1 && visited[i][j][i][j+1]) { // 현재 좌표에 사다리가 있는 경우 
					j++; // 현재 열 + 2 부터 확인하면 됨 
					continue;
				} else if(j+2 < N+1 && visited[i][j+1][i][j+2]) { // 현재 좌표 + 1(오른쪽)에 사다리가 있는 경우 
					j += 2; // 현재 열 + 3부터 확인하면 됨 
					continue;
				} else if(j-1 > 0 && visited[i][j][i][j-1]) { // 현재 좌표 - 1(왼쪽)에 사다리가 있는 경우 
					continue; 
				}
	...
	public static void check(int cnt) {
		for(int i = 1; i < N+1; i++) {
			...
			while(cx != H+1) { // H행을 넘을때까지 사다리 타기 
				if(cy+1 < N+1 && visited[cx][cy][cx][cy+1]) cy = cy+1; // 현재 좌표 오른쪽에 가로선이 있는 경우 -> 오른쪽으로 이동 

				else if(cy - 1 > 0 && visited[cx][cy][cx][cy-1]) cy = cy-1; // 현재 좌표 왼쪽에 가로선이 있는 경우 -> 왼쪽으로 이동 
